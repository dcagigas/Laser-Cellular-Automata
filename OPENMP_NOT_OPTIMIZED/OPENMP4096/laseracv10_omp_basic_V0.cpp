...
//
// THIS IS THE BASIC CODIFICATION (SIMILAR TO THAT OF CUDA)
// HOT ZONES WERE DETECTED AND SEVERAL OPTIMIZATIONS WERE INSERTED
// IN THE OPTIMIZED VERSION laseracv10_omp_optim
//
...
#define SIDE 4096// NUMBER OF CELLS PER SIDE 
...
pcg32_random_t rng_Pumping[SIDE][SIDE]; //one seed per cell
pcg32_random_t rng_PhotonNoise;

laser_e_t  e[SIDE][SIDE];
laser_f1_t f1[SIDE][SIDE], f2[SIDE][SIDE];
laser_lifet_f_t lifet_f[SIDE][SIDE][MAX_PHOT + 1];
...
int main(void)
{
	...
	//-----------------------------------------------------------------------------
#ifdef ENABLE_OMP  
	for (int num_thr = MIN_NUM_THREADS; num_thr <= MAX_NUM_THREADS; num_thr++)
#endif
	{
#ifdef ENABLE_OMP  
...
{
#ifdef ENABLE_OMP  
#pragma omp parallel for default (none) shared (pf1, pf2, lifet_f, e )
#endif
	for (int i = 0; i < SIDE; i++)
	{   
		for (int j = 0; j < SIDE; j++)
		{
			...
			int k = 0;
			while (j2 < n) {
				lifet_f[i][j][k] = lifet_f[i][j][j2] - 1;  
				k += (lifet_f[i][j][k] > 0) ;  
				j2++;
			}
			pf1[i][j] = k;
			...
			if (e[i][j] == 0)					
			{
				...
				else
				{
						if (neighboors(i, j) > threshold)
						{
							if (pf2[i][j] < MAX_PHOT)
							{
								lifet_f[i][j][pf2[i][j]] = lifet_photon;  
								pf2[i][j] = pf2[i][j] + 1;
								e[i][j] = 0;
							}
							else
	...
	void PhotonNoise_v5(void)
	{
		...
		for (int n = 0; n < nphot_noise; n++) {
			...
			if (f1_old < MAX_PHOT) {
				...
				lifet_f[x][y][f1_old] = lifet_photon; 
				pf2[x][y] = pf1[x][y];
			}
			else {
				pf1[x][y] = f1_old - 1;
